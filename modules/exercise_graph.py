...
class ExerciseGraphFrame(ctk.CTkFrame):
    ...
    def __init__(self, master):
        ...
        graph_card.pack(fill="both",expand=True,padx=30,pady=10)

        self.graph_area = ctk.CTkFrame(graph_card,fg_color="white")
        ...
        home_button.pack(padx=40,pady=(5, 15))
    ...
    def show_graph(self):
        ...
        if df is None:
            self.status_label.configure(text="exercise.csv が見つかりません")
        ...
        if df.empty:
            self.status_label.configure(text="運動記録が0件です")
            self.show_message("まだ運動記録がありません。")
        ...
        if "日付" not in df.columns:
            ...
            self.show_message(f"「日付」列がありません。\n\n現在の列名:\n{df.columns.tolist()}")

            return

        if "徒歩(分)" not in df.columns:
            self.status_label.configure(text="CSVの徒歩時間データが見つかりません")
            self.show_message(f"「徒歩(分)」列がありません。\n\n現在の列名:\n{df.columns.tolist()}")
        ...
        if df.empty:
            ...
            self.show_message("日付または徒歩時間を正しく読み込めませんでした。\n\n"
                "徒歩時間には「30」のように数字を入力してください。")

            return

        df = df.sort_values("日付")
        self.status_label.configure(text=f"{len(df)}件の運動記録を表示しています")
        fig = plt.Figure(figsize=(6, 4),dpi=100)
        ax = fig.add_subplot(111)
        ax.plot(df["日付"],df["徒歩(分)"],marker="o")
