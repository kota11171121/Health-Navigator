...
def get_health_advice(prompt):
    ...
    if api_key != "":
        ...
        if bmi < 18.5:
            ...
            advice_list.append(
                "・BMIが低めです。バランスの良い食事を意識しましょう。"
            )

        elif bmi < 25:

            advice_list.append(
                "・BMIは標準的な範囲です。現在の生活習慣を維持しましょう。"
            )

        else:

            advice_list.append(
                "・BMIが高めです。食事や運動の習慣を少しずつ見直してみましょう。"
            )
        ...
        if exercise is None or exercise.empty:
            ...
            advice_list.append(
                "・運動記録がありません。まずは短い散歩などから始めてみましょう。"
            )
        ...
        advice_list.append(
            f"・記録されている平均徒歩時間は "
            f"{round(average_walk, 1)}分です。"
        )

        if average_walk < 10:

            advice_list.append(
                "・運動量が少ない傾向があります。まずは1日10分程度歩くことから始めてみましょう。"
            )

        elif average_walk < 30:

            advice_list.append(
                "・適度に運動できています。少しずつ運動時間を増やしてみましょう。"
            )

        else:

            advice_list.append(
                "・十分な運動を続けられています。これからも無理のない範囲で継続しましょう。"
            )
